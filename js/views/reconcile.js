...
function renderReconcile(c){
  ...
  const activeClose=accountRecord?PocketLedgerPeriodClose.activeFor(DB.accountCloses,accountRecord):null;
  const closeChanges=activeClose?PocketLedgerPeriodClose.changes(activeClose,DB.transactions):[];
  ...
  c.innerHTML = `
    <div class="panel" style="margin-bottom:16px;border-left:3px solid ${activeClose?'var(--income)':'var(--line)'};"><div class="panel-head"><div class="panel-title">Period close<small>${activeClose?`Protected through ${ukDate(activeClose.closedThrough)} · ${closeChanges.length} change${closeChanges.length===1?'':'s'} since close`:'Add a soft guard after completing a statement period'}</small></div>${activeClose?'<button class="btn btn-sm" id="period-reopen">Reopen period</button>':'<button class="btn btn-sm" id="period-close">Close period…</button>'}</div>
      ${activeClose&&closeChanges.length?`<div style="font-size:11.5px;color:var(--gold);">${closeChanges.slice(0,5).map(change=>`${change.kind}: ${escHTML(change.description)}`).join(' · ')}${closeChanges.length>5?` · +${closeChanges.length-5} more`:''}</div>`:activeClose?'<div style="font-size:11.5px;color:var(--income);">No protected transaction has changed since this close.</div>':'<div style="font-size:11.5px;color:var(--ink-faint);">Closing does not alter balances or reconciliation history. It adds an explicit warning before older entries can be edited or deleted.</div>'}
    </div>
    ${stepNav}${body}
  ...
  `;
  const closeButton=document.getElementById('period-close');if(closeButton)closeButton.onclick=()=>{
    openModal(`<div class="modal-head"><h3>Close ${escHTML(account)} through a date?</h3></div><div class="modal-body"><div class="field"><label>Protect transactions through</label><input id="close-through" type="date" value="${statementDate}"></div><div class="field" style="margin-top:10px;"><label>Close note (optional)</label><input id="close-note" type="text" placeholder="e.g. August statement agreed"></div><p style="font-size:11.5px;color:var(--ink-faint);margin:10px 0 0;">A snapshot of transaction fields is stored locally. You can reopen the period later with an audit note.</p></div><div class="modal-foot"><button class="btn" id="m-cancel">Cancel</button><button class="btn btn-primary" id="m-confirm">Close period</button></div>`);
    document.getElementById('m-cancel').onclick=closeModal;document.getElementById('m-confirm').onclick=()=>{try{DB.accountCloses.push(PocketLedgerPeriodClose.create({uid,account:accountRecord,closedThrough:document.getElementById('close-through').value,note:document.getElementById('close-note').value,transactions:DB.transactions}));scheduleSave();closeModal();renderReconcile(c);toast('Period close recorded');}catch(error){toast(error.message,'error');}};
  };
  const reopenButton=document.getElementById('period-reopen');if(reopenButton)reopenButton.onclick=()=>{
    openModal(`<div class="modal-head"><h3>Reopen protected period?</h3></div><div class="modal-body"><p style="font-size:13px;color:var(--ink-soft);margin:0 0 12px;">Transactions through ${ukDate(activeClose.closedThrough)} will become editable again. The close and reopen remain in audit history.</p><div class="field"><label>Reason for reopening</label><input id="reopen-note" type="text" placeholder="e.g. Correcting a late statement entry"></div></div><div class="modal-foot"><button class="btn" id="m-cancel">Cancel</button><button class="btn btn-primary" id="m-confirm">Reopen</button></div>`);
    document.getElementById('m-cancel').onclick=closeModal;document.getElementById('m-confirm').onclick=()=>{const note=document.getElementById('reopen-note').value.trim();if(!note){toast('Enter a reason for reopening','error');return;}activeClose.reopenedAt=new Date().toISOString();activeClose.reopenNote=note;DB.accountCloseAudit.push({id:uid('closeaudit'),closeId:activeClose.id,action:'reopened',at:activeClose.reopenedAt,note});scheduleSave();closeModal();renderReconcile(c);toast('Period reopened');};
  };
