...
function parseJavaScript() {
  ...
  assert(read('js/app.js').includes("const APP_VERSION = '1.25'"));
  assert(read('js/app.js').includes('const SCHEMA_VERSION = 14'));
}
...
function checkPwaAssets() {
  ...
  assert(worker.includes("const CACHE = 'pocket-ledger-v28'"));
  ['css/app.css', 'js/money.js', 'js/app.js', 'js/device.js', 'js/recurring.js', 'js/diagnostics.js', 'js/transfers.js', 'js/reconciliation.js', 'js/start.js', 'js/views/dashboard.js', 'js/views/reconcile.js', 'js/views/health.js',
...
function checkBackupCompatibility() {
  ...
  const empty = () => ({
    schemaVersion: 14, appVersion: '1.25', startingBalance: 0,
    ...
    lastBackupAt: null, lastImport: null, importSessions: [], importProfiles: [], netWorthSnapshots: [], investmentValuations: [],
  });
  const model = context.window.PocketLedgerModel.create({
    ...
    schemaVersion: 14, appVersion: '1.25', money: context.window.PocketLedgerMoney,
  });
  ...
  assert.strictEqual(database.schemaVersion, 14);
...
(async () => {
  ...
  console.log('v1.25 checkpoint checks passed');
