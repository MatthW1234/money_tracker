...
function renderReconcile(c){
  ...
  if(step===1){
    ...
    body=`<div class="panel"><div class="panel-head"><div class="panel-title">Match statement entries<small>“On statement” includes the entry in the calculated balance</small></div></div>
      <div class="table-wrap"><table><thead><tr><th style="width:90px;">On statement</th><th>Date</th><th>Description</th><th>Category</th><th>Status</th><th style="text-align:right;">Amount</th></tr></thead><tbody>${session.transactions.length?session.transactions.map(transaction=>{const status=transactionStatus(transaction),locked=status==='reconciled';return `<tr class="${locked?'reconcile-row-locked':''}"><td><input type="checkbox" data-rec-id="${transaction.id}" ${status!=='pending'?'checked':''} ${locked?'disabled':''} aria-label="${locked?'Previously reconciled':'Appears on statement'}"></td><td>${ukDate(transaction.date)}</td><td class="desc" title="${escAttr(transaction.description)}">${escHTML(transaction.description)}</td><td>${escHTML(transaction.category||'—')}</td><td>${statusPillHTML(status)}</td><td class="amt ${transaction.amount>0?'income':'expense'}">${gbp(transaction.amount,{signed:true})}</td></tr>`;}).join(''):`<tr class="empty-row"><td colspan="6">No transactions fall within this statement period.</td></tr>`}</tbody></table></div>
    </div>`;
  }else if(step===4){
  ...
  const accountEl=document.getElementById('rec-account');if(accountEl)accountEl.onchange=e=>{UI.reconcileAccount=e.target.value;UI.reconcileStatementBalance='';UI.reconcileStartDate='';UI.reconcileWarningsAcknowledged=false;UI.reconcileStep=1;renderReconcile(c);};
...
function completeReconciliation(account,startDate,statementDate,statementBalance,diagnosticWarnings){
  ...
  DB.reconciliations[account].history.push({
    ...
    inflows:session.inflows,outflows:session.outflows,differenceAtCompletion:statementBalance-session.calculatedClosing,
  ...
  UI.reconcileStep=1;UI.reconcileStartDate='';UI.reconcileStatementBalance='';UI.reconcileWarningsAcknowledged=false;scheduleSave(); renderContent(); toast(`Reconciled ${account} to ${label}`);
}
